 ...
 * </pre>
 * 
 * Now Jude is capable of sorting an array (bubble-sort, merge-sort, quick-sort.
 * See the sample codes in <code>/samples</code>). So I believe Jude is now a
 * generally practical compiler to use and now I would like its source code to
 * be visible from everyone.
 * <p>
 * Of course Jude is far from versatile and industrial. It will be improved step
 * by step and currently does not support the following features:
 * <li>float, pointer and reference type,
 * <li>user defined type including String, Enum, Structure, Class etc.,
 * <li>overload of procedure,
 * <li>and many other senior functionalities(IO, Network, Thread and so on).
 *
...
public class Jude extends Helper {
	...
	static boolean isDefinedArray(String name) {
		if (isDefinedGlobalVar(name)) {
			return globalVariables.get(name).length > sizeOfType(globalVariables
					.get(name).type);
		}
		if (isDefinedLocalVar(name)) {
			return localVariables.get(name).size > sizeOfType(localVariables
					.get(name).type);
		}
		if (isParam(name)) {
	...
	static void defineGlobalArray(String name, String type) throws IOException {
		...
		if (look == '=') {
			match('=');
			match('{');
			// The count of initial values should match exactly the size of
			// array:
			int i = 0;
			while (i < size) {
				getToken();
				if (!isConst(info.type, token)) {
					abort("illegal const value:" + token + " of type:"
							+ info.type);
				}
				if (info.type == Type.BOOL) {
					token = boolNumeric(token);
				}
				pushInitCodeLine(TAB + "mov [" + info.name + "+"
						+ sizeOfType(info.type) + "*" + i + "], " + token);
				i++;
				if (i < size) {
					match(',');
				}
			...
			if (i != size) {
				abort("incorrect count of initial values (found " + i
						+ ", expect " + size + ")");
			}
			match('}');
		}
	...
	static void allocaGlobalVar(String name, String type) throws IOException {
		...
		if (look == '=') {
			...
			// TODO let's limit that sophisticated assignment is not permitted
		...
		if (varValue != null) {
			storeCode(name + ":" + TAB + getStoreType(findKeyword(type)) + TAB
					+ varValue);
			pushInitCodeLine("mov " + getAssignType(findKeyword(type)) + " ["
					+ name + "], " + varValue);
		} else {
			storeCode(name + ":" + TAB + getStoreType(findKeyword(type)) + TAB
	 ...
	 * </pre>
	 * 
	 * -> means break the case after block. => means continue the case after
	 * block.
	 */
	static void doCase() throws IOException {
		...
		getToken();
		while (!"}".equals(token)
				&& ("*".equals(token) || isConst(varType, token))) {
			newLine();
			// Default case:
			if ("*".equals(token)) {
				c = '*';
	...
	/**
	 * Print something to terminal: sample code:
	 * 
	 * <pre>
	 * int a = 9, b = 10;
	 * float c = 9.5;
	 ...
	 * </pre>
	 * 
	 * Basic idea is to identify every parameter connected by '+' and use system
	 * calls to print the parameter solely according to its type.<br>
	...
	// TODO It's a little more complicated in printing float variable via
	// syscall. So we skip it as for now.
	...
	static void popProcReg(String methodName) {
		...
		for (Type type : info.params) {
			if (isIntType(type)) {
				intInd++;
				if (intInd <= 6) {
					// pop should be in reversed order:
	...
	static void firstBlock(String word) throws IOException {
		...
		while (keyType != Type.NONE) {
			...
			keyType = findKeyword(token);
			// compileInfo(token);
			// We have to remember whether there was a previous 'if' only when
			// keyword is 'else' or 'elif':
	...
	static void assignment(Type type, String name) throws IOException {
		...
		if (isDefinedArray(name)) {
			...
			emitLn("push rax");
			expression();
			// TODO compare index and array size to optionally generate a
			// runtime exception. Plan is that if out of bound is detected,
	...
	static void storeArray(Type type, String name, String index, Type expType) {
		...
		if (type == Type.FLOAT) {
			...
			if (isDefinedGlobalVar(name)) {
				emitLn("fstp qword [" + name + " + " + index + " * "
						+ sizeOfType(type) + "]");
			} else if (isDefinedLocalVar(name)) {
				emitLn("fstp qword [rbp-" + getLocalVarOffset(name) + " + "
						+ index + " * " + sizeOfType(type) + "]");
			} else {
				abort("expected legal variable, but found " + name);
			}
		} else {
			...
			if (isDefinedGlobalVar(name)) {
				emitLn("push rbx");
				emitLn("lea rbx, [" + name + "]");
				emitLn("mov [rbx + " + index + " * " + sizeOfType(type) + "], "
						+ reg);
				emitLn("pop rbx");
			} else if (isDefinedLocalVar(name)) {
				emitLn("mov [rbp-" + getLocalVarOffset(name) + " + " + index
						+ " * " + sizeOfType(type) + "], " + reg);
			} else {
				abort("expected legal variable, but found " + name);
			}
	...
	static Type loadArray(String name) throws IOException {
		...
		if (isDefinedGlobalVar(name)) {
			emitLn("push rbx");
			emitLn("lea rbx, [" + name + "]");
			emitLn("mov " + reg + ", [rbx + " + index + " * "
					+ sizeOfType(typeOf(name)) + "]");
			emitLn("pop rbx");
		} else if (isDefinedLocalVar(name)) {
			emitLn("mov " + reg + ", [rbp-" + getLocalVarOffset(name) + " + "
					+ index + " * " + sizeOfType(typeOf(name)) + "]");
		} else if (isParam(name)) {
			emitLn("mov " + reg + ", [rbp+" + getParamOffset(name) + " + "
					+ index + " * " + sizeOfType(typeOf(name)) + "]");
		} else {
	...
	static Type loadFloatArray(String name, String index) {
		...
		if (isDefinedGlobalVar(name)) {
			...
			emitLn("fld qword [" + name + " + " + index + " * "
					+ sizeOfType(typeOf(name)) + "]");
			emitLn("fstp qword [" + TEMP_FLOAT_VAR_NAME + "]");
		} else if (isDefinedLocalVar(name)) {
			emitLn("fld qword [rbp-" + getLocalVarOffset(name) + " + " + index
					+ " * " + sizeOfType(typeOf(name)) + "]");
			emitLn("fstp qword [" + TEMP_FLOAT_VAR_NAME + "]");
		} else if (isParam(name)) {
			emitLn("fld qword [rbp+" + getParamOffset(name) + " + " + index
					+ " * " + sizeOfType(typeOf(name)) + "]");
			emitLn("fstp qword [" + TEMP_FLOAT_VAR_NAME + "]");
		} else {
			abort("expected legal variable, but found " + name);
	...
	// extended the value to 64-bit. For floating value, we use a predefined
	// global variable to act as [rax] in integer mode.
	...
	static void boolExpression() throws IOException {
		...
		while (isOrOp(token)) {
			...
			case XOR:
				...
				break;
			default:
				// Actually program would not run into here.
				break;
			}
		}
	}
	...
	static boolean isRelop(String op) {
		return EQUAL.equals(op) || NEQUAL.equals(op) || GREATER.equals(op)
				|| GREATERE.equals(op) || SMALLER.equals(op)
				|| SMALLERE.equals(op);
	}
	...
	public static void main(String[] args) throws IOException {
		...
		openResources(args[0].substring(0, args[0].length() - 5));
		preInitCode();
		codeReader = new BufferedReader(new FileReader(args[0]));
